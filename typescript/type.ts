...
// 인터페이스로 함수의 타입을 지정해도 필요에 따라 매개변수의 내용이 달라진다면 수정할 수 있다
// ex) 자료형, 매개변수의 개수


// 인터페이스의 확장
// 위에 작성한 Person을 확장하여 사용
// extends를 통해 확장, Person에 있는 속성을 적어주지 않아도 Developer에 들어가 있다
interface Position extends Developer {
    // 상속(확장)한 인터페이스를 확장할 수 있다
    // 한 번에 여러개 확장할 수 있다
    // interface Developer extends Person, Position
    // 확장할 때는 이전의 인터페이스에 추가하면 작성한 객체에 오류가 생길 수 있음
    // >> 간으하면 새로 인터페이스를 만들어서 확장한다
    position: string;
}
...
let objectArray: Person[] = [{name:"홍길동"}, {name:"성춘향"}]
